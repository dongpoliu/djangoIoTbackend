...
from .models import AirDetectorData, AirCleaner
...
class UserStats(object):
    ...
    def __init__(self, user):
        self.user = user
        self.data = AirDetectorData.objects.by_user(self.user)
        self.airdetectordata_unit_name = user.settings.airdetectordata_unit.name

    def airdetectordata_by_unit_setting(self, value):
        return core_utils.airdetectordata_by_unit_setting(self.user, value)
    ...
    def latest_entry(self):
        ...
        if latest_entry:
            latest_entry_value = '%s %s' % \
                                 (self.airdetectordata_by_unit_setting(latest_entry.value),
                                  self.airdetectordata_unit_name)
    ...
    def hba1c(self):
        """
        The HbA1c is calculated using the average blood airdetectordata from the last
        90 days.
        ...
        value_html = '%s%%<br><small>(%s %s)</small>' % \
                     (hba1c, self.airdetectordata_by_unit_setting(average),
                      self.airdetectordata_unit_name) \
    ...
    def get_breakdown(self, days=14):
        ...
        average = core_utils.round_value(subset.aggregate(Avg('value'))['value__avg'])
        
        highs = subset.filter(value__gt=self.user_settings['high']).count()
        lows = subset.filter(value__lt=self.user_settings['low']).count()
        within_target = subset.filter(value__gte=self.user_settings['target_min'], value__lte=self.user_settings['target_max']).count()
        other = total - (highs + lows + within_target)

        return {
            'total': total,
            'lowest': {
                'value': '%s %s' % (self.airdetectordata_by_unit_setting(lowest),
                                    self.airdetectordata_unit_name) \
                    if lowest else 'None',
                'css_class': self.get_css_class(lowest),
            },
            'highest': {
                'value': '%s %s' % (self.airdetectordata_by_unit_setting(highest),
                                    self.airdetectordata_unit_name) \
                    if highest else 'None',
                'css_class': self.get_css_class(highest),
            },  
            'average': {
                'value': '%s %s' % (self.airdetectordata_by_unit_setting(average),
                                    self.airdetectordata_unit_name) \
...
class ChartData(object):
    ...
    def get_count_by_category(cls, user, days):
        ...
        category_count = AirDetectorData.objects.by_category(
    ...
    def get_level_breakdown(cls, user, days):
        ...
        airdetectordata_level = AirDetectorData.objects.level_breakdown(
            (now - timedelta(days=days)), now, user)
        ...
        for k, v in sorted(airdetectordata_level.items(),
    ...
    def get_avg_by_category(cls, user, days):
        ...
        airdetectordata_averages = AirDetectorData.objects.avg_by_category(
            (now - timedelta(days=days)), now, user)

        data = {'categories': [], 'values': []}
        for avg in airdetectordata_averages:
            rounded_value = core_utils.round_value(avg['avg_value'])
            data['values'].append(
                core_utils.airdetectordata_by_unit_setting(user, rounded_value))
    ...
    def get_avg_by_day(cls, user, days):
        ...
        airdetectordata_averages = AirDetectorData.objects.avg_by_day(
            (now - timedelta(days=days)), now, user)
...
class AirDetectorDataBaseReport(object):
...
class AirDetectorDataCsvReport(AirDetectorDataBaseReport):

    def generate(self):
        data = AirDetectorData.objects.by_date(
        ...
        try:
            ...
            for item in data:
                row = [
                    self.airdetectordata_by_unit_setting(item.value),
                    item.category,
    ...
    def email(self, recipient, subject='', message=''):
        ...
        attachment_filename = 'AirDetectorDataData_%sto%s.csv' % \
...
class AirDetectorDataPdfReport(AirDetectorDataBaseReport):

    def __init__(self, *args, **kwargs):
        super(AirDetectorDataPdfReport, self).__init__(*args, **kwargs)
    ...
    def generate(self):
        qs = AirDetectorData.objects.by_date(
            self.start_date, self.end_date, self.user)
        ...
        for i in qs:
            value = i.value
            value_by_unit_setting = self.airdetectordata_by_unit_setting(value)

            # Bold the text if the value is high or low based on the user's
            # settings
            low = self.user.settings.airdetectordata_low
            high = self.user.settings.airdetectordata_high
            if value < low or value > high:
        ...
        story.append(Paragraph('AirDetectorData Data', styleH))
    ...
    def email(self, recipient, subject='', message=''):
        ...
        attachment_filename = 'AirDetectorDataData_%sto%s.pdf' % \
                              (self.start_date.strftime(FILENAME_DATE_FORMAT),
                               self.end_date.strftime(FILENAME_DATE_FORMAT))
