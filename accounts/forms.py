...
from devices.models import Category, Unit
...
class SignUpForm(forms.Form):
    ...
    #glucose_unit = forms.ModelChoiceField(Unit.objects.all(), empty_label=None,label='Glucose Unit')
    time_zone = TimeZoneFormField(label=_(u'时区'))
    address  = AddressField(label=_(u'地址') )
...
class UserSettingsForm(forms.Form):
    ...
    #glucose_unit = forms.ModelChoiceField( Unit.objects.all(), label='Glucose Unit', empty_label=None)
    #default_category = forms.ModelChoiceField( Category.objects.all(), label='Default Category',empty_label='Auto', required=False)

    #glucose_low = forms.DecimalField( label='Low', max_digits=6, max_value=3000, min_value=0, help_text="Below this value is a low blood glucose.")
    #glucose_high = forms.DecimalField( label='High', max_digits=6, max_value=3000, min_value=0, help_text="Above this value is a high blood glucose.")
    #glucose_target_min = forms.DecimalField( label='Target Min', max_digits=6, max_value=3000, min_value=0, help_text="Your target range's minimum value.")
    #glucose_target_max = forms.DecimalField( label='Target Max', max_digits=6, max_value=3000, min_value=0, help_text="Your target range's maximum value.")
