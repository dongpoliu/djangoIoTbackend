...
def signup(request):
    if request.method == 'POST':
        ...
        if not form.is_valid():
            ...
            return render_to_response('userauth/signup.html', context)
        else:
        ...
        if request.method == 'POST':
            ...
            if user is not None:
                if user.is_active:
                    ...
                    return render_to_response('userauth/signin.html', context)
            else:
...
@csrf_exempt  
def reset(request):
    return password_reset(request, template_name='userauth/reset.html',
        email_template_name='userauth/reset_email.html',
        subject_template_name='userauth/reset_subject.txt',
        post_reset_redirect=reverse('success'))

@csrf_exempt 
def reset_confirm(request, uidb64=None, token=None):
    return password_reset_confirm(request, template_name='userauth/reset_confirm.html',
